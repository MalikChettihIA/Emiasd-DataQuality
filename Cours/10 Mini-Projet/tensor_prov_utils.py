...
import os
import copy
from tensor_prov import TensorProv
...
def generate_large_df(n_persons=1000, missing_age_fraction=0.2):
    ...
    # Randomly select indices to set as NaN in 'age' column
    num_missing = int(n_persons * missing_age_fraction)  # Calculate number of NaN values
    missing_indices = np.random.choice(persons_df.index, size=num_missing, replace=False)
    persons_df.loc[missing_indices, "age"] = np.nan  # Insert NaNs
...
def execute_and_log(df_counts: list, function, function_args, nb_df: int = 1, base_path="datasets"):
    ...
    if len(first_iteration_results) == 5:
        ...
        print("  Row Indices (Result DataFrame):", hash_cco_matrix.row)
        print("  Col Indices (Original DataFrame):", hash_cco_matrix.col)
        ...
        print("  Row Indices (Result DataFrame):", ids_cco_matrix.row)
        print("  Col Indices (Original DataFrame):", ids_cco_matrix.col)
